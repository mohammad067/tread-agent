...
4. **Two separate LLM calls per run** (never merged): Call #1 = news sentiment scoring from weighted news; Call #2 = explanation synthesis (human summary, ordinal drivers, novelty flags, data gaps). Separation prevents the sentiment score from being biased toward a "nicer narrative."
